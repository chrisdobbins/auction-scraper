...
. .credentials
ITEM_ID="$1"
token_last_updated=$(echo "$(gdate +%s) - $(gdate -r token.txt +%s)" | bc -l)
if [[ ! -n "$2" ]]; then
...
let time_with_offset=$(gdate "+%s")+offset
...
# echo '<p><b>Title:</b> Grundig Yacht Boy 400 Shortwave Radio LW/MW/SW/FM<br><br><b></p>' | sed -E 's/<[a-z]{1,}>/\
# /g;s/<\/[a-z]{1,}>/\
# /g'| tr -s '\n' | sed '/^$/d'
echo "Title: "${title}
...
while [[ $is_bid_enough -eq 1 ]]; do
    let now=$(gdate "+%s")
    let milli_now=$(gdate "+%s%3N")
    let elapsed_time=now-timer
    # let elapsed_milli_time=milli_now-milli_timer
    ...
    if [[ $elapsed_time -ge $polling_interval ]]; then
        ...
        top_bid=$(echo $(echo $response | jq .bidHistory.bidSummary[0].amount | tr -d '"'))
        end_time=$(gdate --date=$(echo $response | jq .endTime | tr -d '"')"PST" "+%s")
        milli_end_time=$(gdate --date=$(echo $response | jq .endTime | tr -d '"')"PST" "+%s%3N")
        ...
        server_time=$(gdate --date=$(echo $response | jq .serverTime | tr -d '"')"PST" "+%s")
        milli_server_time=$(gdate --date=$(echo $response | jq .serverTime | tr -d '"')"PST" "+%s%3N")

        now=$(gdate "+%s")
        milli_now=$(gdate "+%s%3N")

        let time_left=end_time-server_time
        let milli_time_left=milli_end_time-milli_server_time

        let offset=now-server_time
        let milli_offset=milli_now-milli_server_time

        let time_with_offset=$(gdate "+%s")+offset
        let milli_time_with_offset=$(gdate "+%s%3N")-milli_offset

        let local_time_left=end_time-time_with_offset
        let milli_local_time_left=milli_end_time-milli_time_with_offset
    ...
    while [[ ($should_poll_more_1 -eq 1) || ($should_poll_more_2 -eq 1) ]]; do
        ...
        if [[ (${milli_time_left} -gt 0 && ${milli_local_time_left} -gt 0) && (${milli_time_left} -le 500 || ${milli_local_time_left} -le 500) ]]; then
            let should_bid=$(echo "scale=2; ${max_bid}>=${top_bid}+${bid_increment}" | bc -l)
            echo $should_bid
            if [[ $should_bid -eq 0 ]]; then 
                echo "Not bidding; top bid is: "${top_bid}
            fi
        ...
        elif [[ ${milli_time_left} -le 0 || ${milli_local_time_left} -le 0 ]]; then
            echo "auction is over!"
            exit 0
        fi
        # Note: this is non-portable; the man page says that doing this 
        # "will nearly guarantee that a shell script will not properly
        # execute on another system"
        sleep .15
    done
done
